...
from routes.filme_routes import filme_routes             # DE routes.carro_routes IMPORTAR carro_routes: conjunto de rotas do módulo carro

app = Flask(__name__)                                    # Cria a aplicação Flask, passando __name__ para identificar o módulo principal
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///filmes.db'  # Configuração: URL de conexão com o banco de dados SQLite chamado carros.db
...
app.register_blueprint(filme_routes)                     # Registra o blueprint das rotas de carro na aplicação
